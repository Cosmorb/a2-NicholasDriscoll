...
function rowView(r){
  const updatedText = r.updatedAt ? `Updated: ${new Date(r.updatedAt).toLocaleString()}` : '';
  return `
    <tr data-id="${r._id}">
      <td>${r.name}</td>
      <td>${r.email}</td>
      <td>${r.message}</td>
      <td>
        Created: ${new Date(r.createdAt).toLocaleString()}<br>
...
function rowEdit(r){
  return `
    <tr data-id="${r._id}">
      ...
      <td>Created: ${new Date(r.createdAt).toLocaleString()}</td>
      <td>
